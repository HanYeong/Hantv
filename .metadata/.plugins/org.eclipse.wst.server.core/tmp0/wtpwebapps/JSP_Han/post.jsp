		...
		올리기
			<div class="insertdropmenubox">
				<form>
					...
					<div class="dropmenupost" OnClick="location.href='board.jsp'">
						게시글
					</div>
			</div>
		</div>
...
try{
	...
	String sql = "select * from post_db where cid = '" + postid + "'";
	pstmt = conn.prepareStatement(sql);
	rs = pstmt.executeQuery();

	if(rs.next()){
		videoTitle = rs.getString("ptitle");
		...
		sql = "update post_db set viewcnt = viewcnt+1 where cid = '"+postid+"'";
...
<div style="overflow: auto; width:100%-240px; height:100%; background: #f2f4f7;">
	<div class="watchbox">
		...
		<div>
			<form method="post" action="commentpostaction.jsp">
			...
			<input type="hidden" id="videoidhidden" name="videoid" value="<%=postid%>">
...
try{
	...
	String sql = "select * from comment_post_db where postcid = '" + postid + "' order by signdate desc";
	pstmt = conn.prepareStatement(sql);
	rs = pstmt.executeQuery();
	while(rs.next()){
