...
<%
	...
	String uploadPath = request.getRealPath("/resources/contents/videos");
	...
	try{
		...
		title = multi.getParameter("uploadtitle");
		text = multi.getParameter("uploadtext");
...
try{
	...
	if(rs.next()){
		...
		out.println(userimgsrc);
	}
	
	sql = "insert into video_db (userid, vtitle, signdate, comment, videoname, usernick, userimgsrc, viewcnt, thumbscnt) values(?,?,?,?,?,?,?,?,?)";
	pstmt = conn.prepareStatement(sql);	// 쿼리문 수행할 객체 생성
	...
	pstmt.setString(6, usernick);
	pstmt.setString(7, userimgsrc);
	pstmt.setInt(8, 0);
	pstmt.setInt(9, 0);
