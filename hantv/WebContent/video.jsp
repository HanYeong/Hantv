...
<%
	...
	if(sortcnt == 1){
		sortNew = "style='border-bottom:2px solid red; color:#ff5454;'";
	}
	
	if(sortcnt == 2){
		sortView = "style='border-bottom:2px solid red; color:#ff5454;'";
	}
	
	if(sortcnt == 3){
		sortThumbs = "style='border-bottom:2px solid red; color:#ff5454;'";
	}
%>
...
<div class="menutab">
	...
	<div class="menucontents" OnClick="location.href='video.jsp'">
		<div class="menucontents-icon">
			<img src="resources/imgs/hot_selected.png">
		</div>
...
try{
	...
	String sql = "select count(cid) from video_db";
	...
	if(sortcnt == 1){
		sql = "select * from video_db ORDER BY signdate desc, cid desc LIMIT "+first+", "+numPage;
	}else if(sortcnt == 2){
		sql = "select * from video_db ORDER BY viewcnt desc, cid desc LIMIT "+first+", "+numPage;
	}else{
		sql = "select * from video_db ORDER BY thumbscnt desc, cid desc LIMIT "+first+", "+numPage;
	}
	pstmt = conn.prepareStatement(sql);
	rs = pstmt.executeQuery();
	%>
<div style="overflow: auto; width:100%-240px; height:100%; background: #f2f4f7;">
	...
	<div class="contentstab1">
			...
			<div class="contentstypebox">
				<div class="sorttypebox" <%=sortNew %> OnClick="location.href='video.jsp?sort=1'">
					최신순
				</div>
				<div class="sorttypebox" <%=sortView %> OnClick="location.href='video.jsp?sort=2'">
					조회순
				</div>
				<div class="sorttypebox" <%=sortThumbs %> OnClick="location.href='video.jsp?sort=3'">
					인기순
				</div>
			</div>
			<div class="videobox">
				<%
					while(rs.next()){
						%>
						<div class="totalvideo" OnClick="location.href='watch.jsp?video=<%=rs.getString("cid")%>'">
							<div style="width:320px; height: 100%; float: left; background: #1f1f1f;">
							<video src="./resources/contents/videos/<%=rs.getString("videoname") %>#t=5" onmouseover="this.play()" onmouseout="this.pause(), this.currentTime = 5.0" class="videocontents" type="video/mp4" muted="muted">
							</video>
							</div>
	...
	<div style="width:50%; height: 200px; margin: auto; text-align: center; line-height:100px; vertical-align: middle;">
		...
		int directPage = 0;
			if(block > 1){
				myPage = firstPage;
				%>
				<a href="video.jsp?Page=<%=myPage%>&&sort=<%=sortcnt%>">[이전]</a>
				<%
			}else{
				out.println("[이전]");
			}
			for(directPage = (int)firstPage+1;directPage<=lastPage;directPage++){
				if(Page == directPage){
					...
					%>
					<a href="video.jsp?Page=<%=directPage%>&&sort=<%=sortcnt%>">[<%=directPage%>]</a>
					<%
				}
			}
		%>
		...
		<%
			if(block < totalBlock){
				myPage = lastPage+1;
				%>
				<a href="video.jsp?Page=<%=myPage%>&&sort=<%=sortcnt%>">[다음]</a>
				<%
			}else{
				out.print("[다음]");
			}
