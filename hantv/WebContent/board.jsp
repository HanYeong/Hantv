...
<div class="menutab">
	...
	<div class="menucontents" OnClick="location.href='board.jsp'">
		<div class="menucontents-icon">
			<img src="resources/imgs/text_selected.png">
		</div>
...
<div style="overflow: auto; width:100%-240px; height:100%; background: #f2f4f7;">
	...
	<div class="contentstab1">
			...
			<div class="contentstypebox">
				<div class="sorttypebox" <%=sortNew %> OnClick="location.href='board.jsp?sort=1'">
					최신순
				</div>
				<div class="sorttypebox" <%=sortView %> OnClick="location.href='board.jsp?sort=2'">
					조회순
				</div>
			</div>
			<div class="videobox">
				<%
					while(rs.next()){
						%>
						<div class="totalvideo" OnClick="location.href='post.jsp?post=<%=rs.getString("cid")%>'">							
						
							<div style="width:98%; height: 100%;text-align: left; margin-left: 20px;">
								...
								<div class="videouser">
									...
									<div class="videocntbox">조회수 <%=rs.getInt("viewcnt") %></div>
								</div>
							</div>
							</div>
	...
	<div style="width:50%; height: 200px; margin: auto; text-align: center; line-height:100px; vertical-align: middle;">
		...
		int directPage = 0;
			if(block > 1){
				myPage = firstPage;
				%>
				<a href="board.jsp?Page=<%=myPage%>&&sort=<%=sortcnt%>">[이전]</a>
				<%
			}else{
				out.println("[이전]");
			}
			for(directPage = (int)firstPage+1;directPage<=lastPage;directPage++){
				if(Page == directPage){
					...
					%>
					<a href="board.jsp?Page=<%=directPage%>&&sort=<%=sortcnt%>">[<%=directPage%>]</a>
					<%
				}
			}
		%>
		...
		<%
			if(block < totalBlock){
				myPage = lastPage+1;
				%>
				<a href="board.jsp?Page=<%=myPage%>&&sort=<%=sortcnt%>">[다음]</a>
				<%
			}else{
				out.print("[다음]");
			}
